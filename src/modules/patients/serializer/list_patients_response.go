...
type ListPatientsResponseItems = []*ListPatientItem
...
func SerializeListPatient(p *model.Patient) *ListPatientItem {
	if p == nil {
		return nil
	}
...
func SerializeListPatients(list []*model.Patient) ListPatientsResponseItems {
	result := make([]*ListPatientItem, 0, len(list))
	for _, p := range list {
		result = append(result, SerializeListPatient(p))
	}
	return result
}
